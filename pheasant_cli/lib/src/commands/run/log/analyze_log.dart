...
String analyzeLog(String stream) {
  ...
  if (stream.contains('WARNING')) {
    ...
    if (stream.contains('AssetNotFoundException') || stream.contains('Please check the following imports:')) {
      buffer.writeAll([
      ...
      buffer.writeAll([
        ...
        if (stream.split(':')[1].contains('RangeError (index):')) 'There is no data in the file.'
      ]);
    } else {
      buffer.write(newstream);
    }
